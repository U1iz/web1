...
(function () {
    ...
    function logLoadedInfo(msg) {
        ...
        let num_fstSub = 11;
        let msg_tmp01 = '......';
        let num_secSub = max_fontLength - (num_fstSub + 1) - msg_tmp01.length;
        ...
        if (msg.length > max_fontLength) {
            ...
            msg_afterEdit = msg_tmp00 + msg_tmp01 + msg_tmp02;
        }
    ...
    function execute() {
        ...
        for (let i = 0, len1 = img.length; i < len1; i++) {
            (function (j1) {
                img[j1].onload = function () {
                    count++;
                    let imgLoadLog = 'img_' + count + '\0' + getElemUrl(img[i], 'src') + '\0已加载\n';
        ...
        for (let j = 0, len2 = script.length; j < len2; j++) {
            (function (j2) {
                ...
                scriptElem.onload = function () {
                    count++;
                    let scriptLoadLog = 'script_' + count + '\0' + getElemUrl(allScriptElem[j], 'src') + '\0已加载\n';
        ...
        for (let k = 0, len3 = link.length; k < len3; k++) {
            ...
            linkElem.onload = function () {
                count++;
                let linkLoadLog = 'link_' + count + '\0' + getElemUrl(allLinkElem[k], 'href') + '\0已加载\n';
...
setTimeout(function () {
    document.querySelector('#msgCtr').remove();
}, 6400);
window.addEventListener('load', function () {
