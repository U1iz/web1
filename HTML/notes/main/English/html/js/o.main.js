...
function add_btn_cb() {
    document.documentElement.innerHTML += ' <div id="cb_container" style=" position: absolute; top: 0.16rem; right: 12rem; z-index: 6; "> <button style=" position: absolute; padding: 1rem 1.6rem; white-space: nowrap; border-radius: 16px; border: none; box-shadow: 1px 1px 16px 1px #999; " id="cb">关闭伪元素</button> <span id="cb_close" onmouseenter="this.style.backgroundColor = '+"rgba(255,160,170,.6)'" +'onmouseleave='+"this.style.backgroundColor = 'transparent"+'" style=" position: absolute; top: 0; left: 100%; text-align: center; font-size: 0.6rem; /* transform: scale(1.6,1); */ line-height: 1rem; width: 1rem; height: 1rem;/* background-color: rgb(194, 194, 194);*/ ">x</span> </div>';
}
add_btn_cb()
